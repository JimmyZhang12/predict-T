...
VERILOG_TB_PATH="$PREDICT_T_ROOT/power_supply_model"
...
ln -s $VERILOG_TB_PATH/adc.vams .
ln -s $VERILOG_TB_PATH/clk.vams .
ln -s $VERILOG_TB_PATH/dpwm.vams .
ln -s $VERILOG_TB_PATH/pid.vams .
ln -s $VERILOG_TB_PATH/load.vams .
ln -s $VERILOG_TB_PATH/buck.vams .
...
#  buck_model.vams \
#  -top buck_model \
#  -analogcontrol scf.scs | tee "$VERILOG_TB_PATH/log/${1}_out.log"
ncverilog \
	+define+SHM_NAME=\\\"${1}\\\" \
  +define+STEP_SIZE=${2} \
  buck_model.vams \
