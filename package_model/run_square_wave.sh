...
TRACE_DIRS=("desktop_server" "laptop" "mobile" "embedded" "microcontroller" "lp_uc")
...
for i in ${!DEVICES[@]}; do
  for j in ${!SUPPLY[@]}; do
    for k in ${!TRACE_DIRS[@]}; do
      for l in ${!PWM[@]}; do
        for m in $(ls ${TRACE_ROOT}/${TRACE_DIRS[$i]}_trace_${PWM[$l]}/*.csv); do
          # Run 
          if [ $SUPPLY[$j] == "IDEAL" ]; then
